...
- At the top of each run, publish ONE single summary feed entry (just like any other item the tool serves) titled `[summary] <YYYY-MM-DD HH:MM>`. Its body covers all topics defined in `topics.yaml` (`law`, `tech_law`, `global_politics`, `tech`) with a sub-heading per topic, then prose describing the most important things happening in that topic this run. Length is adaptive: short on quiet days, longer when a lot is happening — do not omit important items just to stay brief. Hard cap at 1500 words total. Date the entry so Inoreader shows it at the top before the individual item entries.
